...
> 说明：V1.4.1 已开发完成。实际做法与计划基本一致，
> 细节以代码为准：工具栏按钮沿用现有的 4 个（添加文件/
> 添加目录/移除所选/开始改名），只去掉文字、加入鼠标
> 提示；窗口初始 840×620、最小 800×600；lw=S(70)、
> rh=S(28)；EXT 警告改为单行“※”前缀；DATE 提示宽度
> 按剩余空间计算；状态栏三栏为 W-S(320)/W-S(160)/-1。

- **要解决的问题**：窗口偏小拥挤、左右分组比例失衡、参数区
  标签列宽混用对不齐、工具栏大按钮占地方且无提示。
- **实现路径**（全部在 `src/main.c` 界面代码段，不动改名逻辑）：
  - （已完成 ✔）初始客户区加大（约 840×620 逻辑像素，一律 `S()` 包裹），
    `DoLayout()` 中 `pad` 提至 S(8)、`blockH` 提至 S(160)；
  - （已完成 ✔）工具栏改为纯 16×16 图标小按钮，加 `TBSTYLE_TOOLTIPS`，
    在 `TTN_GETDISPINFOW` 里按按钮 ID 返回“名称＋快捷键”提示，
    原按钮文字全部移入提示；
  - （已完成 ✔）“改名方式”分组框加宽到 S(140)，单选框行距 S(20)；
  - （已完成 ✔）“参数设置”分组框统一标签列宽 lw=S(70)、行距 rh=S(28)，
    “保存方案/删除”按钮按分组框右边缘反推定位，不再写死；
  - （已完成 ✔）文件列表加 `LVS_EX_FULLROWSELECT`，调整 `g_colW[]` 列宽；
  - （已完成 ✔）状态栏三栏按窗口宽度比例计算，窗口缩小时不重叠。
...
> 说明：V1.4.2 已开发完成。树控件的经典线条样式（TVS_HASLINES
> 等）原本已具备，核对无误；后退/前进按钮在工具栏创建时即置灰，
> 导航后由 `UpdateNavButtons()` 恢复；打印前经动态加载的
> `GetDefaultPrinterW`（winspool.drv）检查默认打印机，未安装时
> 给出中文原因与解决办法。
