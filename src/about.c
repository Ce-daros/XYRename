...
static LRESULT CALLBACK AboutProc(HWND h, UINT m, WPARAM w, LPARAM l)
{
    ...
    case WM_CREATE: {
        ...
        AbCtl(h, IDC_ABOUT_TEXT, L"STATIC",
              ...
              L"查找替换、修改扩展名、大小写转换等改名方式，全部操作均可预览，安全\n"
              L"可靠，并支持一键撤销。绿色软件，无需安装，不写注册表，拷贝即用。",
              SS_LEFT, 16, 84, 368, 56, g_abFont);

        AbCtl(h, IDC_ABOUT_COPY, L"STATIC", L"版权所有 (C) 2007 小雨软件工作室　保留所有权利",
              SS_LEFT, 16, 146, 368, 18, g_abFontBold);

        AbCtl(h, IDC_ABOUT_LINE2, L"STATIC", L"", SS_ETCHEDHORZ, 14, 170, 372, 2, NULL);
        ...
        AbCtl(h, IDC_ABOUT_AUTHOR, L"STATIC",
              ...
              SS_LEFT, 16, 180, 368, 72, g_abFont);
        ...
        AbCtl(h, IDC_ABOUT_WARN, L"STATIC",
              ...
              SS_LEFT, 16, 256, 368, 72, g_abFont);

        AbCtl(h, IDC_ABOUT_HOME, L"BUTTON", L"访问主页", BS_PUSHBUTTON, 216, 336, 80, 26, g_abFont);
        AbCtl(h, IDC_ABOUT_OK, L"BUTTON", L"确定", BS_DEFPUSHBUTTON, 306, 336, 80, 26, g_abFont);
        return 0;
    }
